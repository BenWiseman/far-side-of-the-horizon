...
SCRIPTS=(firsttick.R clockweights.R neutrino_masses.R dm_bounds.R lz_expected.R graviton_whichpath.R dp_design.R aic_table.R bmv_phase.R tilt_tensions.R two_port_noise.R cp_wall.R dm_clock.R eta_rp4.R crease_helicity.R)
...
echo ""
if [ "$TOTAL" -ne 96 ]; then
  echo "ERROR: expected 96 RESULT records, received $TOTAL" >&2
  exit 1
fi
