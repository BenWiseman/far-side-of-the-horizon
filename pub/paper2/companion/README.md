...
Fifteen base-R scripts inventory 96 arithmetic comparisons for
`pub/paper2/PAPER2_v3.md` (*The Far Side of the Horizon*). Each reproduces a
...
## Result: 96 / 96 reproduce; 75 / 96 clear the plain 0.5% bar with no loosening at all

Every one of the 96 individual number-comparisons across the 15 R scripts reproduces its
...
**Flat disclosure, stated up front, not buried:** 75 of the 96 comparisons clear the plain
...
- Python: optional `python3` from PATH (override with `PAPER2_PYTHON`). It is **not**
   used for any of the 96 counted R comparisons. The sole Python file,
...
| file | what it is |
|---|---|
| `helpers.R` | shared `report()`/`section()` printer. No physics; only formats an expected-vs-reproduced comparison and emits a `RESULT\|...` line that `run_all.sh` parses. Used by all 15 `.R` scripts. |
| `firsttick.R` | item 1 |
...
| `run_all.sh` | runs the 15 R scripts + the Python pointer script, tallies pass/fail |
| `README.md` | this file |
...
failures (96/96). Under the plain unloosened 0.5% bar, 21 of the 96 miss today — every
...
The release runner also requires every script process to succeed and exactly 96 RESULT records. A process failure or missing output is a failed run.
