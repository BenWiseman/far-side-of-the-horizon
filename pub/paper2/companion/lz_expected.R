...
#   M1     = 4.848e8 GeV        right-handed-neutrino mass, Boyle-Finn-Turok (Paper 2 §4.4)
...
source("helpers.R")
M1 <- 4.848e8; sigma_n <- 1e-72; rho <- 0.4; v <- 230e5; expo_ty <- 2.84
n_dm <- rho / M1                       # cm^-3
...
cat("\nsec 5 table row:\n")
report("expected LZ events (SI q=0, unit efficiency) [count]", expected = 2.3e-30, reproduced = N_coh,
